            ...
            for (let i = 0; i <= description.size.width; i++) { // In order to represent the eastmost walls, the map width is one column greater than the actual maze width
                for (let j = 0; j <= description.size.height; j++) { // In order to represent the southmost walls, the map height is one row greater than the actual maze height
                    ...
                    /* To-do #5 - Create the north walls
                        ...
                        - maze ground is centered on the origin
                        - size of each cell is 1x1(X-dimension, Z-dimension)
                        - The size of each wall is 1 x 1 (X-dimension, Y-dimension); the wall’s Z-dimension (thickness) is
irrelevant
                    ...
                    if (description.map[j][i] == 2 || description.map[j][i] == 3) {
                        wallObject = this.wall.object.clone();
                        wallObject.position.set(0.5 + i - description.size.width / 2.0, 0.5, j - description.size.height / 2.0);
                        this.object.add(wallObject);
                    }
                    ...
                    if (description.map[j][i] == 1 || description.map[j][i] == 3) {
                        wallObject = this.wall.object.clone();
                        wallObject.rotateOnAxis(new THREE.Vector3(0,1,0), Math.PI / 2.0);
    ...
    foundExit(position) {
        ...
        return ... < ... && ... */
        return position.distanceTo(this.exitLocation) < 0.5 * this.scale.x && position.distanceTo(this.exitLocation) < 0.5 * this.scale.z;
    };
}